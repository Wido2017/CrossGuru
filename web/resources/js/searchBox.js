...
String.prototype.format = function() { var txt = this; i = arguments[0].length; while (i--) { txt = txt.replace(new RegExp('\\{' + i + '\\}', 'gm'), arguments[0][i]); } return txt; }
...
function SearchBox(options) {
    ...
    if(options)
    {
        this.defaultAnchor = options.anchor || BMAP_ANCHOR_BOTTOM_RIGHT;// 默认停靠在地图的右下方
        this.defaultOffset = options.offset || new BMap.Size(10, 10);// 默认与地图边界的margin
    }
    else
    {
        this.defaultAnchor = BMAP_ANCHOR_BOTTOM_RIGHT;// 默认停靠在地图的右下方
        this.defaultOffset = new BMap.Size(10, 10);// 默认与地图边界的margin
    }
    this.city = "";// 在哪个城市搜索?
}
